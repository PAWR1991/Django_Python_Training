...
from ..LaR_app.models import User
...
class RecipeManager(models.Manager):
	def addRecipe(self, recipeData, user):
		print(recipeData, user)
		errors = []

		if len(recipeData["name"]) < 1:
			errors.append("Recipe name is required")
		elif len(recipeData["name"]) < 3:
			errors.append("name longer than 2 characters")
		...
		if len(recipeData["ingredients"]) < 1:
			...
			for ingredient in ingredients:
				if len(ingredient.split(" ")) < 3:
					valid = False
			if not valid:
				errors.append("Invalid list of ingredients, you must have at least 3 ingredients with each ingredient seperated by spaces with name and amount units")
